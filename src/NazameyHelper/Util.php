...
namespace Nazamey\Zktecodevices\NazameyHelper;

use Nazamey\Zktecodevices\ZktecoVendor\ZKTeco;
...
class Util
{
  ...
  {
    /**
     * Original nazamey/zkteco implementation reads directly from the low-level socket
     * using internal properties like $_zkclient, $_ip, $_port which do not exist
     * on the Nazamey fork.
     *
     * In the Nazamey package we rely on the higher-level ZKTecoVendor implementation
     * for attendance and users, and only need getSize()/setUser() behaviour from
     * these helpers. To avoid touching non-existent properties, we simply return
     * an empty string here when called.
     */
    return '';
  }
  ...
  {
    // Simplified logger: always write to a local error.log file for this package.
